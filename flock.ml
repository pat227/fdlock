...
module Flock = struct
  ...
  let acquire_internal (alockstruct:lock_) ?(leaseseconds=None) =
    let open Core.Std in 
    ...
    let newt = { pid=(int_of_string (thepid_asstring));
		 ...
		 acquired_dt=(Time.to_string_fix_proto `Utc (Time.now ()));
    ...
    let create_and_write fd =
      ...
      |	2 -> let _ = unlink uniquename in
	     let serialized = Yojson.Safe.to_string (to_yojson newt) in   
    ...
    let readback_verify_truncate_update fd =
	 ...
	 let _ = printf "\nRead_86: %s " s in
	 let lopt = of_yojson (Yojson.Safe.from_string s) in
	 (match lopt with
	 | `Ok l -> 
	    if (l.pid = newt.pid) then (*update time of acquisition*)
	      let serialized = Yojson.Safe.to_string (to_yojson newt) in
	      let _ = single_write fd ~buf:serialized in
	      let _ = ftruncate fd ~len:(Int64.of_int (String.length serialized)) in
	      let _ = printf "\nPIDs match, wrote_90 %s" serialized in
	      true
	    else (*check if lock can be considered expired and take it if so*)
	      (match leaseseconds with
		 Some ls ->
		 let nowtime = Core.Std.Time.now () in
		 let age =
		   Time.diff nowtime (Time.of_string_fix_proto `Utc l.acquired_dt) in
		 ...
		 then (*lock is ours*)
		   ...
		   let _ = single_write fd ~buf:serialized in
		   let _ = ftruncate fd ~len:(Int64.of_int (String.length serialized)) in
		   let _ = printf "\n102_Age of existing lock exceeds max lease \
				   life %d ; acquiring lock" ls in
		   true
		 else (*lock lease still valid...not ours*)
		   let _ = printf "\n108_Age of existing lock less than max lease" in
		   false
	       | None -> let _ = printf "\n110_Lock lease still valid..." in false)
	 | `Error s -> let _ = printf "\n126_failed to marshall from json b/c:%s\n" s in raise Unrecognized_lock_state)
    ...
    try 
      ...
      | Error _ -> let _ = with_file ~perm:0o600 ~mode:[O_EXCL;O_CREAT;O_RDWR]
		   ...
		   let lopt = of_yojson (Yojson.Safe.from_string readback) in
		   (match lopt with
		   | `Ok l -> if (idendtt l newt) then true else false
		   | `Error s -> let _ = printf "139_failed to marhsall from json b/c %s\n" s in raise Unrecognized_lock_state)
      | Ok () -> with_file ~perm:0o600 ~mode:[O_RDWR] alockstruct.path
			   ~f:readback_verify_truncate_update
    with
    | MyEExist -> let _ = printf "\n125_File exists...reading it see if it \
				  belongs to us..." in
    ...
    | Impossible_lock2large -> let _ = printf "\n130_Lock file size too large for \
					       int type. U R SOL." in false
  ...
  let release_ (alockstruct:lock_) =
    ...
    let readback_verify fd =
      ...
      | Some size -> let s = String.create size in
		     ...
		     let lopt = of_yojson (Yojson.Safe.from_string s) in
		     (match lopt with
		      | `Ok l -> if (l.pid = (Pid.to_int (Unix.getpid ()))) then true else false
		      | `Error s -> let _ = printf "\n165_error, failed to marshall from json b/c:%s\n" s in raise Unrecognized_lock_state)
      | None -> raise Impossible_lock2large in
    let r = access alockstruct.path [`Read;`Write;`Exists] in
    match r with
    | Ok () -> let _ = printf "\n149_Releasing lock file, it exists...confirm it \
			       belongs to us first" in
  ...
  let acquire (lockt:t) leaseseconds =
    let internallock = { path = lockt.path; status = Unlocked } in
    acquire_internal internallock ~leaseseconds;;
end
